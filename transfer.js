...
const calcDisplayBalance = function (acc) {
  ...
  labelBalance.textContent = `${acc.balance} Rwf`;
};
...
const calcDisplaySummary = function (acc) {
  ...
  labelSumIn.textContent = `${incomes} Rwf`;

  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = `${Math.abs(out)} Rwf`;
  ...
  labelSumInterest.textContent = `${interest} Rwf`;
};
